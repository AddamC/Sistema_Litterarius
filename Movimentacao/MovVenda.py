...
import Banco
from Movimentacao.ConfRecebimento import ConfRecebimento
from UI import Venda_ui
from Movimentacao.MovBuscarLivro import MovBuscarLivro

class Venda(QDialog):
    def __init__(self):
        super(Venda, self).__init__()
        self.ui = Venda_ui.Ui_Form()
        ...
        self.conf_pagamento = ConfRecebimento()
    ...
    def clickedContinuar(self):
        ...
        if self.conf_pagamento.confirmado:
            ...
            for i in range(self.ui.twCarrinho.rowCount()): # para cada item no carrinho
                livro = carrinho.item(i, 0).text()
                qtde = carrinho.item(i, 1).text()
                preco = carrinho.item(i, 2).text()
                Banco.inserirDetVenda(qtde, preco, livro) #faz a baixa no estoque
    ...
    def carregarComboBox(self):
        ...
        if db.open ():
            ...
            model.select ()
            self.ui.cbxClientes.setModel (model)
            self.ui.cbxClientes.show ()
        db.close ()
